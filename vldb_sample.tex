...
\begin{figure*}
\centering
\includegraphics[width=0.9\textwidth]{figures/easeml}
\vspace{-1em}
...
shows the formal syntax of the DSL.


For example, if a user wants to train a classifier
for RGB images into 3 classes, he/she would submit a job specifying the
schema:
\vspace{-0.5em}
\[
\texttt{Input = [256, 256, 3]
...
\eml then automatically matches all {\em consistent} machine
learning models (e.g., AlexNet, ResNet-18, GoogLeNet, etc.) 
that can be used for user's application, and
explore all these models. Whenever a new model
finishes running, \eml returns the model.
The user does not know {\em when}
and {\em where} a model is trained, and for them 
\eml is just a ``view'' of the best available model.




More specifically, after a user submits his/her job with the schema specification,
\eml will (1) perform schema matching and task generation for the job, (2) do some profiling to estimate the cost of the job and submit the job by putting it into a user-level task pool, and (3) allocate computation resource and run the job.
\eml automatically manages the underlying infrastructure (i.e., computation resources and storage) that is shared by multiple users.
At any time, \eml tries to maximize the global ``happiness'' of all of its users (we will formalize this in Section~\ref{sec:multitenant}).
That is, each user will be able to run the best model found so far for his/her task at any given time.

Given the inherent multi-tenant nature of the \eml design, the system employs an
automatic scheduler to prioritize the execution of
different models for different users. The key technical question
here is {\em how can we come up with an execution strategy
(i.e., one execution order of all models) to maximize the
``global happiness'' of all of our users?} 
In the next section, we formalize
...
values of $k$. Each $k$ generates one additional
 model.}
\end{figure}
...
\begin{theorem}
  ...
  Given $\delta\in (0,1)$ and a sequence of positive numbers $\{c_k^i\}_{k=1}^{K^i}$,
  and let $\{\beta_t^i\}$ be a solution of \eqref{eq:beta-multi}, then the
  total regret of Multi-tenant GP-UCB is bounded above by
  \[
    C_2 n \sqrt{T}\sqrt{\sum_{i=1}^n I([T(i)])}
  \]
  ...
  \]
  with $c^\ast = \max_{i,k}\{c^i_k\}, \beta^\ast =\max_{i,t}\{\beta^i_t\}$ and $\sigma^\ast= \max_{i}\{\sigma^i\}$.
\end{theorem}

In particular, if the kernel function or covariance matrix is linear (see
  ...
  \]
  In this case, the total regret is bounded by
  \[
    C_2 n^{3/2}\sqrt{T \log\left(\frac{T}{n}\right)}.
  \]
