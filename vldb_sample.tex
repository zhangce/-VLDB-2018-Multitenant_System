...
we started from two lines of classic research:
(1) classic single-tenant model selection with 
\emph{multi-armed bandits}~\cite{XXX,XXX,XXX};
and (2) classic {\em multi-task} Bayesian optimization
and Gaussian Process~\cite{XXX,XXX,XXX} dealing
with multiple players.
When extending these approaches to \eml's multi-tenant case, 
we faced two major challenges:

\noindent
{\bf (Optimization Objective)} The single-tenant optimization objective 
is simple: Find the best model for the user as soon as 
possible (i.e., to minimize user's {\em regret} accumulated over time). 
When extending this objective to multiple players,
previous work proposes different objectives for different tasks:
Take Swersky et al.'s recent work~\cite{XXX} for example ---
the objectives and algorithms it designed for 
(1) k-fold cross validation (each fold
is a ``tenant'') and (2) using cheaper dataset 
to help expensive dataset (each dataset is a ``tenant'')
are different.
Unfortunately, as we will discuss in Section~\ref{sec:relatedwork},
both objectives are not suitable for \eml's multi-tenant
scenario. The first challenge is to design a reasonable 
objective for multiple users and design novel
algorithms for such an objective. Moreover,
...
To our best knowledge, \eml is one of the very first systems 
of its kind and our first contribution is the system design and the formulation
...
We formalize
this as a technical problem that 
...
From theortical perspective, we proved a rigorous 
multi-tenant regret bound. 
...
\begin{algorithmic}[1]                    % enter the algorithmic environment
   ...
   \STATE Observe $y_t = f(a_t) + \epsilon_t$\\
...
a_t = \argmax_{k\in [K]}\left\{\mu_{t-1}(k) + \sqrt{\frac{\beta_t}{c_k}}\sigma_{t-1}(k)\right\}.
\end{equation}
...
\begin{theorem}
  \label{thm:cost}
  Given $\delta\in (0,1)$ and set $\beta_t = 2c^\ast \log\left[\frac{\pi^2 K t^2}{6\delta}\right]$ with $c^\ast = \max_{k\in [K]}\{c_k\}$. Then the total regret of the single tenant, cost-aware GP-UCB is bounded above by
  \[
  \sqrt{T\cdot I(T)}
  \]
%   Then the
%   simple regret of the single 
...
\]
\end{theorem}
Note that $I(T)$ is bounded by $XXX$ \cite{}. This theorem suggests that the regret at iteration $t$ (the minimal regret up to iteration $t$) converges to $0$ with respect to the running time $\sum_{t=1}^T c_{a_t}$ with high probability. 
...
When there are multiple tenants are available, we can only pick one tenant to run at each round. Our goal is to minimize the cummunative regret 
\[
define the regret over here.
\]
Note that this regreat is not the sum of regrets of all tenants, since at any time point the regret is computed from current regrets from all tenents. It means that if we do not pick tenant $i$ at round $t$, then its regeat at round $t-1$ will be undertaken at round $t$ again.

The most straightforward idea is applying the FIFS principle: we serve each tenant until find out its optimal algorithm before move to the next one. Unfortunately, it gives a very terrible cummunative regret:
\[
xx
\]

\subsubsection{Round Robin GP-UCB}
A simple idea to avoid the embarrasement is to apply the round robin priciple, that is, each tenant will be selected alternatively. The native approach admits the following cummunative regret.
...
\begin{theorem}
  ...
  \[
     \sqrt{n T}\sum_{i=1}^n \sqrt{I_i([T(i)])}
  \]
  with probability at least $1-\delta$,
  where
  \begin{align*}
    I_i([T(i)]) & = \frac{4 c^\ast \beta^\ast}{\log(1 + (\sigma^\ast)^{-2})}\\
...
\begin{itemize}
\item When the runing time goes infinity, the regret will converge to zero;
\item If assume the cost for all algorithms to be identical, then the regret at time $t$ decreases to $0$ with a rate $\frac{n^1.5}{T^0.5}$.
\end{itemize}
...
It is observed that $I_i([T(i)])$ is proportional to the information gain for each user $i$. In particular, if the kernel function or covariance matrix is linear (see
 ...
 n^{3/2}\sqrt{T \log\left(\frac{T}{n}\right)}.
  \]
For other two common used kernel functions: the Squared Exponential kernel and the Mat\'{e}rn kernel, we can also get a bound, which is sublinear in $T$. We refer to Section 5.2 in \cite{SrinivasKKS10} for details.
...
all systems are aware of the underlying cost,
a more realistic scenario that \eml is designed for.
For the \textsc{DeepLearning} dataset, we use the real cost (execution time) of each model.
For the \textsc{179Classifier} dataset and the four synthetic datasets, we generate costs randomly.
...
is because of the quality of GP estimator ---
As the execution proceeds, it is more
and more important for GP to have a good
estimation of the model quality to choose
the next user to serve. When all users'
current accuracy are high-enough, the modeling
...
\vspace{-1em}
\paragraph*{Discussion: Single- vs. Multiple- Devices}
One design decision we made in \eml, which reflects
in our experiment protocol is that we are
treating all GPUs we have as {\em a single device}.
The rational behind this decision is that 
we are able to obtain linear scale up to
29GPUs for all of our models. 
Consider a multi-device alternative:
With 29 users and we allocate one GPU for each user.
Both alternatives require the same 
amount of GPU-time to finish training, however,
the single-device strategy returns model
faster for {\em some} users. In fact,
for our \textsc{DeepLearning} service,
single-device achieves better 
accumulated regret among users 
than the multi-device alternative.
However, we do realize that this design
decision is not fundamental and it is 
important future work to make \eml aware of
multiple devices in the resource pool.
...
\begin{figure}[t!]
\centering
\includegraphics[width=0.3\textwidth]{figures/impact_train_nocost}
\vspace{-2em}
...
From the system perspective, \eml is closely related to
AutoML systems that recently built by both the database and machine learning community.
...
are build upon state-of-the-art automatic model selection and 
hyper-parameter tuning algorithms such as GP-UCB~\cite{SrinivasKKS10},
GP-PI~\cite{Kushner1964}, GP-EI~\cite{SnoekLA12}, and 
Thompson sampling~\cite{ChapelleL11,KaufmannKM12,Scott:2010}.
See a recent survey by Luo~\cite{Luo16} for an overview of this area.
Compared with these systems, \eml is an AutoML system based on GP-UCB, however, focus
on the multi-tenant case when multiple users share the same
infrastructure running machine learning workload. To our best knowledge,
...
\begin{proof}
  ...
  \begin{align*}
    ...
    & \le \sqrt{\frac{\beta^i_{t^i}}{c^i_{a^i_{t}}}}\sigma^i_{t-1}(a^i_{t^i}) +\mu^i_{t-1}(a^i_{t^i}) - f^i(a^i_{t_i})\\
    & \le 2
...
\begin{align*}
  ...
  Therefore,
  \[
    \sum_{t=1}^T \sum_{i=1}^n c^i_{a^i_t} r^i_{t^i} \le \sqrt{n T}\sum_{i=1}^n \sqrt{I_i([T(i)])}.
  \]
\end{proof}
