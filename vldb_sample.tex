...
The well-known cumulative regret of UCB algorithm \cite{~} is $R_T \leq C\cdot K\log T$,
where $C$ is some constant depending on the arm distribution. It is known to be one of the best upper bounds for multi-bandit problem. This bound has a minor dependence on $T$, but seriously depends on the distribution of all arms and the number of arms. This is mainly because the UCB algorithm does not consider dependence between arms. In order to make $R_T/T$ converge to zero, we have to try as least $K$ times. So the UCB algorithm requires to play all arms for once or twice in the initial step. To utilize the dependence among arms, previous work has extended the UCB algorithm with Gaussian process, where the dependence among arms can be measured by a Kernel matrix. The GP-UCB algorithm can achieve the regret
\[
R_T \leq C\cdot \sqrt{T\log T}
\]
where $C$ is some constant number, not depending on the distribution of arms. We can see that this bound does not depend on the number of arms, but a worse dependence on $T$. GP-UCB does not require to pull all arms once to initialized the algorithm. It can achieve a satisfactory average regret before all arms get pulled. GP-UCB is suitable for the variance of each arm is small, but the number of arms is large or pulling an arm is time-expensive.
...
multi-tenant setting as it is one of the critiera to balance between
multiple users.
...
this cost-aware algorithm. Despite of the simplicity of the theoretical
analysis and that we do not claim technical contribution for
this analysis, nevertheless we did not see similar algorithm and analysis in the literature.
